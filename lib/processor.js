...
function getCollection(clientReqMsg) {
  var dbPieces = clientReqMsg.fullCollectionName.split('.');
  var dbName = dbPieces.shift();
  var collectionName = dbPieces.join('.');
  logger.info('getCollection', util.format('that.mocks[%s][%s]', dbName, collectionName));
  return that.mocks[dbName][collectionName] || [];
}
...
that = {
          ...
          if (clientReqMsg.fullCollectionName.match(/\.\$cmd$/)) {
            if (clientReqMsg.query) {
              if (clientReqMsg.query.findandmodify) {
                ...
                that.doCreateIndexes(socket, clientReqMsg);
                break;
              }
            }
  ...
  doCreateIndexes: function(socket, clientReqMsg) {
    logger.info('doCreateIndexes');
