...
function buildValue(selectors, value) {
  ...
  return buildValue(selectors.slice(0, -1), newValue);
}

function setDocumentValue(doc, mongoSelector, value) {
  var formatDocTraversErrorMessage = function(selector) {
    return 'cannot use the part (' +
      selector + ' of + ' + mongoSelector + ') to traverse the document ' + doc;
  }
  var setValue = function(doc, selectors, value) {
    ...
    if (Array.isArray(doc)) {
      if (selector.match(/^[0-9]$/)) {
        ...
        if (doc.length > selector) {
          if (remainingSelectors.length > 0) {
            setValue(doc[selector], remainingSelectors, value);
          } else {
            doc[selector] = value;
          }
        } else {
          ...
          doc.push(buildValue(remainingSelectors, value));
        }
      } else {
      ...
      doc[selector] = buildValue(remainingSelectors, value);
    }
  }
  setValue(doc, mongoSelector.split('.'), value);
}
      ...
      for (updateKey in clientReqMsg.update) {
        if (updateKey === '$set' || updateKey == '$inc') {
          for (propKey in clientReqMsg.update[updateKey]) {
            ...
            if (updateKey == '$set') {
              setDocumentValue(doc, propKey, value);
            } else {
