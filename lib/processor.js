...
ServerConnection.prototype.processProtocolMessage = function processProtocolMessage(header, buf) {
  var clientReqMsg;
  var Command;
      ...
      if (clientReqMsg.fullCollectionName.match(/\.\$cmd$/)) {
        if (clientReqMsg.query) {
          if (clientReqMsg.query.findandmodify) {
            ...
            Command = Aggregate;
            break;
          } else if (clientReqMsg.query.insert) {
            Command = Insert;
            break;
          } else if (clientReqMsg.query.update) {
            Command = Update;
            break;
          } else if (clientReqMsg.query.delete) {
            Command = Delete;
            break;
          } 
        }
...
ServerConnection.prototype.processSocketData = function processSocketData(buf) {
  ...
  if (this.accumulatedBuffer) {
    buf = Buffer.concat([this.accumulatedBuffer.buffer, buf])
    header = this.accumulatedBuffer.header
  } else {
    header = protocol.fromMsgHeaderBuf(buf);
  }
  if (buf.length >= header.messageLength) {
     delete this.accumulatedBuffer;
     this.processProtocolMessage(header, buf)
  } else {
    // wait for the remaining data.
    this.accumulatedBuffer = {header: header, buffer: buf}
  }
};
